...
async def create_wordpress_blog_post(
    ...
    try:
        ...
        post_content = f"""<div class="kirin-vulnerability-post">

<div class="vuln-header">
<h2>🚨 {vulnerability.title}</h2>

<div class="vuln-metadata">
<span class="badge severity-{analysis.severity.lower()}"><strong>{analysis.severity} SEVERITY</strong></span>
<span class="badge cvss-score"><strong>CVSS {analysis.cvss_score}/10</strong></span>
<span class="badge patch-status"><strong>{analysis.patch_status.upper()}</strong></span>
</div>
</div>
...
{"<div class='kirin-plugin-notice'><h3>🔧 Kirin Plugin Available</h3><p><strong>Good News!</strong> This vulnerability can be automatically remediated using the Kirin security plugin for supported AI development environments.</p></div>" if vulnerability.kirin_remediation_available else ""}
...
<!-- WordPress Categories: Security, Vulnerability, AI Tools, {analysis.severity} -->
<!-- WordPress Tags: {", ".join(analysis.affected_tools)}, {", ".join(analysis.attack_vectors)} -->
"""
