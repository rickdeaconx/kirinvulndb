...
async def create_wordpress_blog_post(
    ...
    try:
        ...
        post_content = f"""
<div class="kirin-vulnerability-post">
    <div class="vuln-header">
        <h2>🚨 Security Alert: {vulnerability.title}</h2>
        <div class="vuln-metadata">
            <span class="badge severity-{analysis.severity.lower()}">{analysis.severity} SEVERITY</span>
            <span class="badge cvss-score">CVSS {analysis.cvss_score}/10</span>
            <span class="badge patch-status">{analysis.patch_status.upper()}</span>
        </div>
    </div>
    ...
    <div class="vuln-impact">
        ...
        <ul>
            {"".join(f"<li>{vector.replace('_', ' ').title()}</li>" for vector in analysis.attack_vectors)}
        </ul>
    </div>

    <div class="affected-tools">
        <h3>🛠️ Affected AI Tools</h3>
        <ul>
    ...
    {"<div class='kirin-plugin-notice'><h3>🔧 Kirin Plugin Available</h3><p>This vulnerability can be automatically remediated using the Kirin security plugin for supported AI development environments.</p></div>" if vulnerability.kirin_remediation_available else ""}

    {"<div class='references'><h3>📚 References</h3><ul>" + "".join(f"<li><a href='{ref}' target='_blank' rel='noopener'>{ref}</a></li>" for ref in (vulnerability.references or [])) + "</ul></div>" if vulnerability.references else ""}

    <div class="vuln-footer">
        <p><strong>AI Analysis Confidence:</strong> {analysis.confidence_score * 100:.0f}%</p>
        {"<p><strong>Reporter:</strong> " + reporter_name + "</p>" if reporter_name else ""}
        <p><em>This analysis was generated by Knostic AI's Kirin Vulnerability Database. For the most up-to-date information, visit our <a href='http://localhost:8080/api/vulnerabilities/{vulnerability.vulnerability_id}'>vulnerability database</a>.</em></p>
    </div>
</div>

<!-- WordPress Categories -->
<!-- Security, Vulnerability, AI Tools, {analysis.severity} -->
<!-- Tags: {", ".join(analysis.affected_tools)}, {", ".join(analysis.attack_vectors)} -->
"""
